...
Before setting up KLEMA, ensure you have the following installed on your system:

- **Node.js** 18.0+ and npm (or yarn)
- **PHP** 8.2+ with required extensions (pdo, pdo_pgsql, mbstring, xml, curl, zip, gd)
...
```bash
git clone https://github.com/NuxPower/sia-project
...
3. **Configure your `.env` file** with the following essential settings:
   ...
   VITE_APP_ENV=local
   # Vite dev server URL for hot-reloading (use your local IP for network access)
   VITE_DEV_SERVER_URL=http://localhost:5173
   # For local network access:
   # VITE_DEV_SERVER_URL=http://192.168.1.18:5173
   ...
   # For local network access, set to your machine's IP
   SESSION_DOMAIN=localhost
   # For local network access:
   # SESSION_DOMAIN=192.168.1.18

   # Laravel Sanctum Configuration
   # Stateful domains for API authentication (add your local IP for network access)
   SANCTUM_STATEFUL_DOMAINS=127.0.0.1:5173,localhost:5173,localhost:8000
   # For local network access, add your IP:
   # SANCTUM_STATEFUL_DOMAINS=127.0.0.1:5173,localhost:5173,192.168.1.18:5173,192.168.1.18:8000
   # Token expiration in minutes (default: 240 = 4 hours)
   SANCTUM_EXPIRATION=240
   # Remember token expiration in minutes (default: 43200 = 30 days)
   SANCTUM_REMEMBER_EXPIRATION=43200

   # Capacitor Mobile App Configuration
   # Server URL for mobile app development (use your local IP for device testing)
   CAPACITOR_SERVER_URL=http://localhost:8000/app
   # For mobile device testing on same network:
   # CAPACITOR_SERVER_URL=http://192.168.1.18:8000/app
   ...
   **Important Configuration Notes:**
   ...
   - **Database**: Create a PostgreSQL database named `klema` (or your preferred name)
   - **OpenWeatherMap API**: Obtain an API key from [openweathermap.org](https://openweathermap.org/api)
     - Use the same key for both `OPENWEATHER_API_KEY` and `VITE_OPENWEATHER_API_KEY`
   - **Email Setup**: 
     - For Gmail: Use an [App Password](https://support.google.com/accounts/answer/185833) (not your regular password)
     - For SendGrid: Obtain an API key from [sendgrid.com](https://sendgrid.com)
   - **Local Network Access**: 
     - Replace `localhost` with your machine's IP address (e.g., `192.168.1.18`) if you need to access from other devices on the same network
     - Find your IP: `hostname -I` (Linux) or `ipconfig` (Windows) or `ip addr show` (Linux)
   - **Mobile Development**: 
     - Set `CAPACITOR_SERVER_URL` to your local IP when testing on physical devices
...
The application will be accessible at `http://localhost:8000/app`.
...
4. **Configure Capacitor for local development:**
   
   Update `capacitor.config.ts` to point to your local server:
   ```typescript
   server: {
     url: 'http://YOUR_LOCAL_IP:8000/app',
     cleartext: true
   }
   ```
...
4. **Configure Capacitor for local development:**
   ...
   Update `capacitor.config.ts` similar to Android setup with your local IP address.

5. **Build and run:**
   - In Xcode, select your target device and click "Run"
   - Ensure your Mac and iOS device are on the same network

#### Quick Build Script (Android)

For a quick build and install on Android:
...
1. **Database Connection Error:**
   ...
   - Ensure database exists: `psql -l | grep klema`
...
4. **Permission Errors:**
   ...
   - Fix ownership if needed: `chown -R www-data:www-data storage bootstrap/cache`

5. **Mobile App Not Connecting to Backend:**
   - Ensure backend server is running
   - Verify IP address in `capacitor.config.ts` matches your machine's IP
   - Check firewall settings
   - Ensure mobile device and development machine are on the same network
